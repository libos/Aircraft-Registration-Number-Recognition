//--------------------------------------------------------------------------------------------------
// Linear time Maximally Stable Extremal Regions implementation as described in D. Nist√©r and
// H. Stew√©nius. Linear Time Maximally Stable Extremal Regions. Proceedings of the European
...
variation_(numeric_limits<double>::infinity()), stable_(false), parent_(0), child_(0), next_(0)
{
	...
	//ptseq = cvCreateSeq(CV_SEQ_ELTYPE_POINT, sizeof(CvSeq), sizeof(CvPoint) , storage);
	//pointArray = NULL;
	//pointNumber= 0;
	maxX = maxY =0;
	minX = minY =10000;
...
poolIndex_(0)	//http://ivan4126.blog.163.com/blog/static/209491092201301534928785/
{
	// Parameter check
	assert(delta > 0);
	assert(minArea >= 0.0);
...
void MSER::processStack(int newPixelGreyLevel, int pixel, vector<Region *> & regionStack)
{
	...
	do {
		...
		// here that the top of stack should be considered one ‚Äôtime-step‚Äô back, so its current
...
void MSER::operator()(const uint8_t * bits, int width, int height, vector<Region> & regions)
{
	...
	regionStack.push_back(new (&pool_[poolIndex_++]) Region);//Ãÿ∂®µÿ÷∑
	...
	int curPixel = 0;
	int curEdge = 0;
	int curLevel = bits[0];	// grayscaleµ⁄“ªŒª
	accessible[0] = true;
	
	// 3. Push an empty component with current level onto the component stack.
step_3:
	regionStack.push_back(new (&pool_[poolIndex_++]) Region(curLevel, curPixel));//level=>µ±«∞ª“∂»÷µ£¨curPixelµ±«∞µ„
	
	if (poolIndex_ == pool_.size())		//»Áπ˚pool∑≈≤ªø™¡À£¨¥Û–°*2
		doublePool(regionStack);
